...
<p>invoice:{{ $dataTransaksis->invoice}}<br>
pelanggan:{{ $dataTransaksis->pelanggan->nama}}<br>
Cashier:{{ Auth::user()->nama}}<br>
Tanggal:{{$dataTransaksis->created_at->format('d M Y H:m')}}</p>
<table>
    <thead>
        <tr>
            <td>Produk</td>
...
@foreach($datadetil_transaksi as $dod)
<tr>
